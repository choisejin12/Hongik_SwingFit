...
const LoadingContext = createContext({
    ...
    loading: {start: () => {}, stop: () => {}}//spinner`: 스피너(로딩 표시기)를 제어하기 위한 객체. `start` 및 `stop` 메서드를 제공하여 스피너를 시작하거나 중지할 수 있음
});
...
const LoadingProvider = ({children}) => {
    ...
    const value = {inProgress,loading}; //`value` 객체에 `inProgress`와 `spinner`를 포함
    return (
    //`ProgressContext.Provider`를 렌더링하고 하위 컴포넌트(`{children}`)에 컨텍스트 값을 제공
    <LoadingContext.Provider value={value}> 
        {children}
    </LoadingContext.Provider>)//value에 진행중인 작업 여부 및 로딩제어기 넘겨줌
};
